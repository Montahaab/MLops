...
def main():
    ...
    mlflow.set_tracking_uri(
        "http://localhost:5000"
    )  # Spécifie l'URL de ton serveur MLflow

    parser = argparse.ArgumentParser(
        description="Pipeline de machine learning pour la prédiction de churn."
    )
    parser.add_argument(
        "--data_path",
        type=str,
    ...
    parser.add_argument(
        "--predict", action="store_true", help="Faire des prédictions avec le modèle."
    )
    ...
    X_train, X_test, y_train, y_test, _ = (
        ...
        if args.prepare or args.train or args.evaluate or args.predict
    ...
    with mlflow.start_run():
        ...
        if args.train:
            if X_train is None or y_train is None:
                print(
                    "Erreur : Les données doivent être préparées avant l'entraînement."
                )
                return
            ...
            print(
                f"Model logged with n_estimators={rf_model.n_estimators} and max_depth={rf_model.max_depth}"
            )
        ...
        if args.evaluate:
            ...
            if model:
                ...
                if isinstance(accuracy, (int, float)):
                    ...
                    print("Error: Accuracy is not a valid number.")
